...
func book(user *airline.User) error {
	...
	var seatID int
	row := txn.QueryRow("SELECT id FROM seats WHERE trip_id = 1 AND user_id IS NULL ORDER BY id LIMIT 1 FOR UPDATE")
...
func main() {
	...
	fmt.Printf("RESULT approach=2 users=%d success=%d no_seat=%d failures=%d assigned=%d duration_ms=%d\n",
